function [idx, src, dst] = generateSpanningTrees(A, srcs, dsts, nodes, hub1, hub2)
...
%         disp('************************');
%         disp(size(idx_map(a)));
        validated = validate_tree(idx_map(a), hub1, hub2, srcs, dsts, nodes);
        if validated == 1
            disp("test")
...
function b = validate_tree(tr, hub1, hub2, srcs, dsts, nodes)
    list_of_nodes_with_at_least_two_degree = [];
    disp(size(tr))
    for tr_idx = 1:size(tr, 1)
        local_node = tr(tr_idx, 1);
        if local_node == hub1
            continue
        elseif local_node == hub2
            continue
        else
            n1 = count_value_in_list(local_node, srcs);
            n2 = count_value_in_list(local_node, dsts);
            if n1 + n2 >= 2
                list_of_nodes_with_at_least_two_degree = [list_of_nodes_with_at_least_two_degree local_node];
            end
        end
    end
    
    if size(list_of_nodes_with_at_least_two_degree, 2) == size(nodes, 2) - 2 % 2 is the number of hubs
       b = 1;
       disp("welcome")
    else
       b = 0;
...
function numbers = count_value_in_list(val, list)
    numbers = 0;
    for ix = 1:size(list, 2)
        if list(ix) == val
            numbers = numbers + 1;
        end
    end
